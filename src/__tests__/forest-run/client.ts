...
import { makeVar, PossibleTypesMap } from '../../cache';
...
describe("client", () => {
  ...
  itAsync(
    ...
    (resolve, reject) => {
      ...
      const client = new ApolloClient({
        link,
        cache: new ForestRunCache({ addTypename: false }),
      });
  ...
  itAsync("store can be rehydrated from the server", (resolve, reject) => {
    ...
    const client = new ApolloClient({
      link,
      cache: new ForestRunCache({ addTypename: false }).restore(
        initialState.data,
      ),
    });
    ...
    return client
      ...
      .then((result) => {
        ...
        expect(finalState.data).toEqual(
          (client.cache as ForestRunCache).extract(),
        );
      })
      .then(resolve, reject);
  });
  ...
  itAsync("should be able to transform queries", (resolve, reject) => {
    ...
    const client = new ApolloClient({
      link,
      cache: new ForestRunCache({ addTypename: true }),
    });
  ...
  describe("accepts dataIdFromObject option", () => {
    ...
    itAsync("for internal store", (resolve, reject) => {
      ...
      return client
        ...
        .then((result) => {
          expect(result.data).toEqual(data);
          expect((client.cache as ForestRunCache).extract()["1"]).toEqual({
            id: "1",
            name: "Luke Skywalker",
          });
        })
  ...
  describe("standby queries", () => {
    itAsync(
      ...
      (resolve, reject) => {
        ...
        const client = new ApolloClient({ link, cache: new ForestRunCache() });
