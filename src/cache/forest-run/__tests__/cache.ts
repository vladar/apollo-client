...
import { InMemoryCacheConfig } from "../../inmemory/types";
...
describe("Cache", () => {
  function itWithInitialData(
    ...
    callback: (...caches: ForestRunCache[]) => any,
  ) {
...
describe("resultCacheMaxSize", () => {
  ...
  it("configures max size on caches when resultCacheMaxSize is set", () => {
    ...
    new ForestRunCache({ resultCacheMaxSize });
    expect(wrapSpy).toHaveBeenCalled();
...
describe("InMemoryCache#broadcastWatches", function () {
  ...
  it("should pass WatchOptions through to cache.diff", () => {
    ...
    const canonicalCache = new ForestRunCache({
      canonizeResults: true,
      typePolicies,
    });

    const nonCanonicalCache = new ForestRunCache({
