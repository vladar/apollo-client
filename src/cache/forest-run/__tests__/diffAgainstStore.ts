...
import { defaultDataIdFromObject } from "../../inmemory/helpers";
import {
  NormalizedCache,
...
import { withError } from "../../inmemory/__tests__/helpers";

disableFragmentWarnings();

function defaultNormalizedCacheFactory(seed?: NormalizedCacheObject): any {
  ...
  const store: any = cache;

  // Just for tests-compat:
  if (!store["lookup"]) {
    store["lookup"] = function lookup(id: string) {
...
function writeQueryToStore(options: any) {
  const {
    dataId = "ROOT_QUERY",
    cache = defaultNormalizedCacheFactory(),
    ...writeOptions
  } = options;
...
describe("diffing queries against the store", () => {
  const reader = {
    diffQueryAgainstStore(options: any) {
      const cache = options.store;
      return cache.diff({ returnPartialData: true, ...options });
    },
  };
  const writer = {};
  // const writer = new StoreWriter(cache);

  it("expects named fragments to return complete as true when diffd against the store", () => {
  ...
  it("expects inline fragments to return complete as true when diffd against the store", () => {
    const store = defaultNormalizedCacheFactory();
  ...
  // @forest-run: doesn't work with design? (possible to do with global read)
  it.skip("does not error on a correct query with union typed fragments", () => {
    return withError(() => {
      const firstQuery = gql`
        query {
          person {
      ...
      // TODO: @readFrom(query="firstQuery", variables={})
      const unionQuery = gql`
        query {
          person {
            __typename
  ...
  describe("referential equality preservation", () => {
    ...
    // @forest-run: doesn't support field policies; TODO: support at least for Query type
    it.skip("will preserve equality with custom resolvers", () => {
      const listQuery = gql`
        {
          people {
            id
