...
import { hasOwn } from '../../inmemory/helpers';
import { ForestRunCache } from "forest-run";

describe('fragment matching', () => {
  it('can match exact types with or without possibleTypes', () => {
    const cacheWithoutPossibleTypes = new ForestRunCache({
      addTypename: true,
    });

    const cacheWithPossibleTypes = new ForestRunCache({
      addTypename: true,
      possibleTypes: {
        Animal: ['Cat', 'Dog'],
      },
...
describe("policies.fragmentMatches", () => {
  ...
  itAsync("can infer fuzzy subtypes heuristically", (resolve, reject) => {
      ...
      visit(fragments, {
        FragmentDefinition(frag) {
          function check(typename: string, result: boolean) {
            if (result !== cacheInMemory.policies.fragmentMatches(frag, typename)) {
              reject(`fragment ${
                frag.name.value
    ...
    cacheInMemory.policies.addPossibleTypes({
      C: ["TooLong"],
    });
